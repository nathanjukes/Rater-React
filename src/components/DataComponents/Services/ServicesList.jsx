...
const ServicesList = ({ services, onPageChange, appId, group }) => {
  ...
  useEffect(() => {
    if (services) {
      ...
      const servicesWithParentApp = sortedServices.map((service) => {
        const parentApp = service.flatStructure
          ? service.flatStructure.split("/")[1]
          : "";
        return { ...service, parentApp };
      });
      setServices(servicesWithParentApp);
    }
  }, [services]);
  ...
  const groupedServices = useMemo(() => {
    ...
    return servicesList.reduce((grouped, service) => {
      const { parentApp } = service;
      if (!grouped[parentApp]) {
        grouped[parentApp] = [];
      }
      grouped[parentApp].push(service);
